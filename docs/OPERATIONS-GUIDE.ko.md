...
- **immediate**: true로 설정하면 글로벌 큐를 우회하여 즉시 실행
...
제한 초과 시 새 run은 QUEUED 상태로 대기하며, 이전 run 완료 시 자동으로 다음 run이 실행됩니다.
...
Admin UI (스크립트 목록 → Delete 버튼) 또는 REST API로 스크립트를 삭제할 수 있습니다:

```bash
curl -X DELETE http://host:18080/api/publisher/scripts/my-script
```

스크립트 디렉터리와 모든 버전이 삭제됩니다. 실행 중인 run에는 영향 없습니다.

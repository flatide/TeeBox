...
- **명단** — `dataDir/users/users.json`, `{"username": ..., "role": "admin"|"monitor"|"user"}` JSON 배열.
...
- **사용자 관리 UI** — **admin** 으로 로그인하면 **Users** 메뉴(`/admin/users`)가 보입니다:
  사용자 추가(역할 `user`/`monitor`/`admin`, 초기 비밀번호 선택 지정), 역할 변경, 비밀번호 초기화(임시
...
- **로그인이 막는 것** — **`/admin` UI 전체**, GET 페이지 포함(로그인 페이지만 접근 가능).
  ...
  등록한 스크립트만 수정/실행할 수 있고, run 의 kill/cancel 및 디버깅은 **본인이 TeeBox UI 에서
  직접 실행한 run 만** 가능합니다. 또한 본인 Version Source(저장 전 에디터 버퍼 포함)를

...
| `clientJar` | `build/libs/teebox-client-<version>.jar` — the embeddable zero-dependency client packaged as a jar (Java 8 bytecode, no deps) for hosts that prefer a dependency over copying the source. Genuine Java 7 hosts still embed the source. |
